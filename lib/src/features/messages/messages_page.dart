...
class MessageNotifier extends ChangeNotifier {
  ...
  /// Show native notification for new message
  Future<void> showNewMessageNotification(
      String roomName, String message) async {
  ...
  Future<void> _refreshChatRooms() async {
    ...
    try {
      ...
      if (currentCount != _lastKnownMessageCount &&
        ...
        // Find the room with the new message
        for (final room in rooms) {
          if (room.lastMessage != null) {
            ...
            if (room.lastMessage!.senderId != currentUserId) {
              // New messages arrived from others - update the notification count
              MessageNotifier().setUnreadCount(1);
              // Show native notification for the latest message
              MessageNotifier().showNewMessageNotification(
                room.eventTitle,
        ...
        // Update count so we don't notify again for the same messages
